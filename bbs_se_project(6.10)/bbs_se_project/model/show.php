...
if(!$_GET['action']) {
	$sql = "update Post set PHits = PHits + 1 where PID = $pub_id";
	$conn->query($sql);
}
...
$rowsPerPage = 5;
// 查询总记录数
$sql = "select count(*) as sum from Reply where RPostID=$pub_id";
...
$rep_sql = "select * from Reply where RPostID = $pub_id order by RTime limit $offset, $rowsPerPage";
