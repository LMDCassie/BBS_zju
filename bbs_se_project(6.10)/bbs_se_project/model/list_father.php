...
$session_name = $_GET['new_session'];
// 查询总记录数
$sql = "select count(*) as sum from Post where PSession=$session_name";
...
$sql = "select * from Post where PSession=$session_name order by PTime desc limit $offset, $rowsPerPage";
$result = $conn->query($sql);
...
if($session_name==1){
	include '../view/list_father_1.html';
}elseif($session_name==2){
	include '../view/list_father_2.html';
}elseif($session_name==3){
	include '../view/list_father_3.html';
}elseif($session_name==4){
	include '../view/list_father_4.html';
}
